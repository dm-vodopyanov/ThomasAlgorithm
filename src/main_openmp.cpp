...
void help() {
	printf("This is a Thomas algorithm demo application [OPENMP].\n\n");
...
void writeResultToFile(char* fileName, vector<double> x, int N) {
	FILE* outputFile;
	outputFile = fopen("outputfile_openmp", "wt");
...
void writeTimeToFile(double resultTime) {
	FILE* timeFile;
	timeFile = fopen("timefile_openmp", "wt");
	fprintf(timeFile, "%lf\n", resultTime);
	fclose(timeFile);
}
...
#pragma omp parallel for shared(matrix, X) private(y) firstprivate(x, grainsize)
		for (y = 0; y < P; y++)
		{
			...
			if(y != 0)
				X[grainsize * (y + 1) - x - 1] = (matrix[(grainsize * (y + 1) * 4 - 1) - x * 4] - g[grainsize * (y + 1) - x]  * X[grainsize * (y + 1) - x] - d[grainsize * (y + 1) - x] * X[grainsize * (y + 1) - x - grainsize]) / matrix[(grainsize * (y + 1) * 4 - 3) - x * 4];
			else	
				X[grainsize * (y + 1) - x - 1] = (matrix[(grainsize * (y + 1) * 4 - 1) - x * 4] - g[grainsize * (y + 1) - x] * X[grainsize * (y + 1) - x]) / matrix[(grainsize * (y + 1) * 4 - 3) - x * 4];
		}
	}
	//}
	// ADD YOUR CODE HERE
